...
(function (window) {
    ...
    NotificationClient.defaultConfig = {
        ...
        linkStoreLimit: 10, // Giới hạn số lượng thông báo dạng link lưu trong localStorage
        debug: false,
    ...
    NotificationClient.prototype._updateLinkCount = function (sourceId) {
        ...
        let linkCountActive = 0;
        if(linkCount > 0){
          linkCountActive =  containerLink.querySelectorAll('a[data-show-count="0"]').length;
        }

        // Cập nhật số lượng vào container
        linkCountContainer.textContent = linkCountActive;
    ...
    NotificationClient.prototype._applyLinkStoreLimit = function () {
        var limit = parseInt(this.config.linkStoreLimit, 10);
        ...
        for (var i = 0; i < this.notifications.length; i++) {
            ...
            var isLinkType = notification && notification.showTypes && notification.showTypes.indexOf(3) !== -1;

            if (isLinkType) {
                linkNotifications.push(notification);
            } else {
                otherNotifications.push(notification);
            }
        }

        if (linkNotifications.length === 0) {
            this.notifications = otherNotifications;
            return;
        }

        linkNotifications.sort(function (a, b) {
            return self._getDateCreatedValue(b) - self._getDateCreatedValue(a);
        });

        var totalLinks = linkNotifications.length;
        var keptLinks = totalLinks;

        if (linkNotifications.length > limit) {
            trimmed = true;
            linkNotifications = linkNotifications.slice(0, limit);
            keptLinks = limit;
        }

        if (trimmed) {
            this._debug('Link store limit applied:', limit, 'Total link notifications:', totalLinks, 'Kept:', keptLinks);
        }

        this.notifications = linkNotifications.concat(otherNotifications);
    };

    NotificationClient.prototype._getDateCreatedValue = function (notification) {
        if (!notification) {
            return 0;
        }

        var raw = notification.dateCreated;

        if (!raw && notification.lastUpdated) {
            raw = notification.lastUpdated;
        }

        if (!raw && notification.startDate) {
            raw = notification.startDate;
        }

        return this._coerceTimestampToMs(raw);
    };

    NotificationClient.prototype._coerceTimestampToMs = function (value) {
    ...
    NotificationClient.prototype._formatAbsoluteLinkTime = function (date) {
    ...
    NotificationClient.prototype._getRelativeTimeText = function (date) {
        ...
        if (diff < hour) {
            var minutesAgo = Math.max(1, Math.floor(diff / minute));
            return minutesAgo + ' phút trước';
        }

        if (diff < day) {
            var hoursAgo = Math.max(1, Math.floor(diff / hour));
    ...
    NotificationClient.prototype._formatLinkDisplayTime = function (date) {
        if (!date || isNaN(date.getTime())) {
            return '';
        }

        var relativeText = this._getRelativeTimeText(date);
        if (relativeText) {
            return relativeText;
        }

        return this._formatAbsoluteLinkTime(date);
    };

    NotificationClient.prototype._getLinkDisplayTimeText = function (notification) {
        if (!notification) {
            return '';
        }
        var timestamp = this._getDateCreatedValue(notification);

        if (!timestamp && notification.lastUpdated) {
            timestamp = this._coerceTimestampToMs(notification.lastUpdated);
        }

        if (!timestamp && notification.startDate) {
            timestamp = this._coerceTimestampToMs(notification.startDate);
        }
        ...
        return this._formatLinkDisplayTime(date);
    };

    NotificationClient.prototype._getLinkAbsoluteTimeText = function (notification) {
        if (!notification) {
            return '';
        }
    ...
    NotificationClient.prototype._showLink = function (notification) {
        ...
        var linkLabel = notification.title || 'Thông báo';
        var linkTimeText = this._getLinkDisplayTimeText(notification);
        var linkAbsoluteTimeText = this._getLinkAbsoluteTimeText(notification);
        var finalLabel = linkTimeText ? linkLabel + ' <br/> <span style="font-size: smaller; color: gray;">' + linkTimeText + '</span>' : linkLabel;
        linkElement.innerHTML  = finalLabel;
        if (linkTimeText) {
            linkElement.setAttribute('data-link-time', linkTimeText);
        }
        if (linkAbsoluteTimeText) {
            linkElement.setAttribute('data-link-time-absolute', linkAbsoluteTimeText);
            linkElement.title = linkAbsoluteTimeText;
        }
        ...
        this._setLinkShowCountAttribute(notificationId, linkElement);
    ...
    NotificationClient.prototype._updateNotificationStatus = function (notificationId) {
        ...
        this._setLinkShowCountAttribute(notificationId);
    ...
    NotificationClient.prototype._getNotificationShowCount = function (notificationId) {
        if (this.status[notificationId] && typeof this.status[notificationId].shownCount === 'number') {
            return this.status[notificationId].shownCount;
        }
        return 0;
    };

    NotificationClient.prototype._setLinkShowCountAttribute = function (notificationId, linkElement) {
        var element = linkElement || document.getElementById('notification-link-' + notificationId);
        if (!element) {
            return;
        }

        var showCount = this._getNotificationShowCount(notificationId);
        element.setAttribute('data-show-count', showCount);
        element.setAttribute('data-has-shown', showCount > 0 ? 'true' : 'false');
    };
